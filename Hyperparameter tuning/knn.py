...
features.drop(features[features['wt(secs)'].astype(str) == 'unk'].index, inplace = True)
features.drop(features[features['wt(secs)'].astype(str) == 'Error'].index, inplace = True)
...
clf = GridSearchCV(
        KNeighborsClassifier(), parameters, scoring='accuracy', n_jobs= 20
)
