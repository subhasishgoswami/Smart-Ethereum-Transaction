...
model_knn = KNeighborsClassifier(p=1, weights= 'uniform', algorithm= 'auto', n_neighbors= 3, n_jobs= -1)
model_knn.fit(train_features, train_labels)
predictions_knn = model_knn.predict(test_features)
import sklearn
print(sklearn.metrics.accuracy_score(test_labels, predictions_knn)*100, '%')
from sklearn.metrics import f1_score
serial_number= [serial_number for serial_number in range(486342)]
true_data = pd.DataFrame(data = {'serial_number': serial_number, 'actual': test_labels})
predictions_data = pd.DataFrame(data = {'date': [serial_number for serial_number in range(486342)], 'prediction': predictions_knn})
